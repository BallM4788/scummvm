...
void N3D_C3D_ImmDrawEnd() {
	return C3D_ImmDrawEnd();
}

void N3D_C3D_ImmDrawRestartPrim() {
	return C3D_ImmDrawRestartPrim();
}
...
C3D_RenderTarget *N3D_C3D_RenderTargetCreateFromTex(C3D_Tex* tex, GPU_TEXFACE face, int level, C3D_DEPTHTYPE depthFmt) {
	C3D_RenderTarget *tmp = C3D_RenderTargetCreateFromTex(tex, face, level, depthFmt);
	debug("N3D_C3D_RenderTargetCreateFromTex result: %ld", (u32)tmp);
	return tmp;
}
