...
#ifndef GRAPHICS_3DS_3D_CONTEXT_H
#define GRAPHICS_3DS_3D_CONTEXT_H
...
class ShaderObj {
public:
	ShaderObj(u32 *shbin, u32 shbin_size);
	// Copy constructor
	...
	bool setUniform(N3DContext *context, const Common::String &uniform, GPU_SHADER_TYPE shaderEnum, const Math::Matrix<R,C> &svmMtx) {
		int columns = C;
		int pos = getUniformLocation(uniform, shaderEnum);
		if (pos != -1) {
			if (C > 1) {
				if (context->activeShaderObj == this) {
					C3D_Mtx m;
					Mtx_Zeros(&m);
				...
				return setUniform(context, uniform, shaderEnum, temp[0], temp[1], temp[2], temp[3]);
			}
		} else {
			return false;
		}
	...
	bool setUniform(N3DContext *context, const Common::String &uniform,
	                GPU_SHADER_TYPE shaderEnum, const C3D_Mtx &c3dMtx, int rows = 4) {
		int pos = getUniformLocation(uniform, shaderEnum);
		if (pos != -1) {
			if (context->activeShaderObj == this) {
	...
	bool setUniform(N3DContext *context, const Common::String &uniform,
	                GPU_SHADER_TYPE shaderEnum, float x, float y = 0.0f, float z = 0.0f, float w = 0.0f) {
		int pos = getUniformLocation(uniform, shaderEnum);
		if (pos != -1) {
			if (context->activeShaderObj == this) {
	...
	bool setUniform(N3DContext *context, const Common::String &uniform, GPU_SHADER_TYPE shaderEnum, const C3D_FVec &c3dfvec) {
		return setUniform(context, uniform, shaderEnum, c3dfvec.x, c3dfvec.y, c3dfvec.z, c3dfvec.w);
	}

	// uniform vector arrays (floats)
	bool setUniformNfv(N3DContext *context, const Common::String &uniform,
	                   GPU_SHADER_TYPE shaderEnum, float *dataPtr, int vecCount, int floatsPerVec) {
		int pos = getUniformLocation(uniform, shaderEnum);
		if (pos != -1) {
			if (context->activeShaderObj == this) {
	...
	bool setUniform4fv(N3DContext *context, const Common::String &uniform,
	                   GPU_SHADER_TYPE shaderEnum, float *dataPtr, int vecCount) {
		return setUniformNfv(context, uniform, shaderEnum, dataPtr, vecCount, 4);
	}
	bool setUniform3fv(N3DContext *context, const Common::String &uniform,
	                   GPU_SHADER_TYPE shaderEnum, float *dataPtr, int vecCount) {
		return setUniformNfv(context, uniform, shaderEnum, dataPtr, vecCount, 3);
	}
	bool setUniform2fv(N3DContext *context, const Common::String &uniform,
	                   GPU_SHADER_TYPE shaderEnum, float *dataPtr, int vecCount) {
		return setUniformNfv(context, uniform, shaderEnum, dataPtr, vecCount, 2);
	}
	bool setUniform1fv(N3DContext *context, const Common::String &uniform,
	                   GPU_SHADER_TYPE shaderEnum, float *dataPtr, int vecCount) {
		return setUniformNfv(context, uniform, shaderEnum, dataPtr, vecCount, 1);
	}

	// vector uniform values (int)
	bool setUniform(N3DContext *context, const Common::String &uniform,
	                GPU_SHADER_TYPE shaderEnum, int x, int y = 0, int z = 0, int w = 0) {
		int pos = getUniformLocation(uniform, shaderEnum);
		if (pos != -1) {
			if (context->activeShaderObj == this) {
	...
	// bool uniform
	bool setUniform(N3DContext *context, const Common::String &uniform, GPU_SHADER_TYPE shaderEnum, bool boolval) {
		int pos = getUniformLocation(uniform, shaderEnum);
		if (pos != -1) {
			if (context->activeShaderObj == this) {
	...
	void freeUnnamedBuffer(/*C3D_BufInfo *bufInfo, bool inLinearMem, */int bufInfoIdx = 0);
...
#endif // GRAPHICS_3DS_3DCONTEXT_H
