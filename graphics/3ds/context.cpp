...
void N3DContext::init(N3DContext *source) {
	if (source != nullptr) {
		...
		cullFace_faceToCull     = ENUM3DS_CULLFACE_BACK;     // C3D default
		cullFace_frontFace      = ENUM3DS_FRONTFACE_CCW;     // C3D default
...
void N3DContext::initOGL() {
	...
	cullFace_faceToCull     = ENUM3DS_CULLFACE_BACK;
	cullFace_frontFace      = ENUM3DS_FRONTFACE_CCW;
...
void N3DContext::applyContextState(bool forceApply) {
	...
	if ((dirtyFlags & kDirtyCulling) || forceApply) {
		...
		if (cullFace_enabled == true)
			mode = ((cullFace_faceToCull == ENUM3DS_CULLFACE_FRONT) ^ (cullFace_frontFace == ENUM3DS_FRONTFACE_CCW))
	...
	if ((dirtyFlags & kDirtyDepthTest) || forceApply)
		C3D_DepthTest(depthTest_enabled, depthTest_func, depthTest_writeMask);
