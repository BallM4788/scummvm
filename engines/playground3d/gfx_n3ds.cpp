...
void N3DSRenderer::init() {
	...
	_cubeVBO = custom3DS_CreateBuffer(sizeof(float) * 11 * 24, cubeVertices, 0x4);
	...
	_offsetVBO = custom3DS_CreateBuffer(sizeof(float) * 3 * 6, triOffsetVertices, 0x4);
	_offsetShader->addAttrLoader(0, GPU_FLOAT, 3);
	_offsetShader->addBufInfo(_offsetVBO, 3 * sizeof(float), 1, 0x0);

	_fadeVBO = custom3DS_CreateBuffer(sizeof(float) * 2 * 4, dimRegionVertices, 0x4);
	...
	_viewportVBO_1 = custom3DS_CreateBuffer(sizeof(float) * 2 * 4, boxVertices, 0x4);
	_viewportVBO_2 = custom3DS_CreateBuffer(sizeof(float) * 2 * 4, boxVertices2, 0x4);
	...
	_bitmapVBO = custom3DS_CreateBuffer(sizeof(float) * 4 * 4, bitmapVertices, 0x4);
...
void N3DSRenderer::loadTextureRGBA(Graphics::Surface *texture) {
	...
	void *tmp = custom3DS_CreateBuffer(wPow2 * hPow2 * 4);
	...
	// custom3DS_DataToBlockTex and custom3DS_ArbDataToArbBlockTexOffset will swap bytes to
	...
	custom3DS_DataToBlockTex((u32 *)tmp,               0, 0, wPow2, hPow2,
	                         (u32 *)_textureRgba.data, 0, 0, wPow2, hPow2,
	                         wPow2, hPow2, GPU_RGBA8);

	// Flush the texture changes.
	C3D_TexFlush(&_textureRgba);
	// Delete the temporary buffer.
	custom3DS_FreeBuffer(tmp);
}
...
void N3DSRenderer::loadTextureRGB(Graphics::Surface *texture) {
	...
	// Only create the temporary buffer AFTER initializing the C3D_Tex,
	//	so as to prevent fragmentation of linear memory.
	void *tmp = custom3DS_CreateBuffer(wPow2 * texture->h * 4);
	...
	//	before swizzling. Input pixels are assumed to be big endian.
	custom3DS_DataToBlockTex((u32 *)tmp,              0, 0, wPow2, hPow2,
	                         (u32 *)_textureRgb.data, 0, 0, wPow2, hPow2,
	                         wPow2, hPow2, GPU_RGBA8);
...
void N3DSRenderer::loadTextureRGB565(Graphics::Surface *texture) {
	...
	//	before swizzling. Input pixels are assumed to be big endian.
	custom3DS_DataToBlockTex((u32 *)tmp,                 0, 0, wPow2, hPow2,
	                         (u32 *)_textureRgb565.data, 0, 0, wPow2, hPow2,
	                         wPow2, hPow2, GPU_RGBA8);
...
void N3DSRenderer::loadTextureRGBA5551(Graphics::Surface *texture) {
	...
	//	C3D_SyncDisplayTransfer instead of custom3DS_DataToBlockTex.
	...
	// Reverse components, as C3D_SyncDisplayTransfer does not do any byte
	//	swapping itself.
	u16 *texPtr;
	byte *tmpPtr;
	for (int y = 0; y < texture->h; y++) {
		for (int x = 0; x < texture->w; x++) {
			texPtr = (u16 *)texture->getBasePtr(x, y);
			tmpPtr = (byte *)tmp + (y * wPow2 * 4) + (x * 4);
			tmpPtr[0] = ((*texPtr & 0x0001) >>  0) * 255;		// A - NOTE ABGR ORDER
			tmpPtr[1] = ((*texPtr & 0x003E) >>  1) / 31 * 255;	// B - NOTE ABGR ORDER
			tmpPtr[2] = ((*texPtr & 0x07C0) >>  6) / 31 * 255;	// G - NOTE ABGR ORDER
			tmpPtr[3] = ((*texPtr & 0xF800) >> 11) / 31 * 255;	// R - NOTE ABGR ORDER
		}
	}
	...
	// NOTE: C3D_SyncDisplayTransfer seems to cause a hardware hang for data
	//	representing pixels numbering 32x32 or less.
...
void N3DSRenderer::loadTextureRGBA4444(Graphics::Surface *texture) {
	...
	for (int y = 0; y < texture->h; y++) {
		for (int x = 0; x < texture->w; x++) {
			texPtr = (u16 *)texture->getBasePtr(x, y);
			tmpPtr = (byte *)tmp + (y * wPow2 * 4) + (x * 4);
			tmpPtr[0] = ((*texPtr & 0x000F) >>  0) / 15 * 255;	// A - NOTE ABGR ORDER
			tmpPtr[1] = ((*texPtr & 0x00F0) >>  4) / 15 * 255;	// B - NOTE ABGR ORDER
			tmpPtr[2] = ((*texPtr & 0x0F00) >>  8) / 15 * 255;	// G - NOTE ABGR ORDER
			tmpPtr[3] = ((*texPtr & 0xF000) >> 12) / 15 * 255;	// R - NOTE ABGR ORDER
		}
	}
...
void N3DSRenderer::drawCube(const Math::Vector3d &pos, const Math::Vector3d &roll) {
	...
	glTo3DS_BlendFunc(GPU_ONE, GPU_ZERO);
...
void N3DSRenderer::dimRegionInOut(float fade) {
	...
	glTo3DS_BlendFunc(GPU_ONE, GPU_ONE_MINUS_SRC_ALPHA);
...
void N3DSRenderer::drawRgbaTexture() {
	...
	glTo3DS_BindTexture(0, &_textureRgba);
	N3DS_3D::getActiveContext()->applyContextState();
	C3D_DrawArrays(GPU_TRIANGLE_STRIP, 0, 4);
	...
	offset.setY(-0.8f);
	_bitmapShader->setUniform("offsetXY", GPU_VERTEX_SHADER, offset);
	glTo3DS_BindTexture(0, &_textureRgb);
	C3D_DrawArrays(GPU_TRIANGLE_STRIP, 0, 4);

	offset.setX(0.2f);
	offset.setY(-0.8f);
	_bitmapShader->setUniform("offsetXY", GPU_VERTEX_SHADER, offset);
	glTo3DS_BindTexture(0, &_textureRgb565);
	C3D_DrawArrays(GPU_TRIANGLE_STRIP, 0, 4);

	offset.setX(0.7f);
	offset.setY(-0.8f);
	_bitmapShader->setUniform("offsetXY", GPU_VERTEX_SHADER, offset);
	glTo3DS_BindTexture(0, &_textureRgba5551);
	C3D_DrawArrays(GPU_TRIANGLE_STRIP, 0, 4);

	offset.setX(-0.8f);
	offset.setY(-0.2f);
	_bitmapShader->setUniform("offsetXY", GPU_VERTEX_SHADER, offset);
	glTo3DS_BindTexture(0, &_textureRgba4444);
	C3D_DrawArrays(GPU_TRIANGLE_STRIP, 0, 4);
}
