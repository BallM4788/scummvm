...
public:
	...
	void drawStart(u8 flags = 0, u32 x = 0, u32 y = 0, u32 w = 640, u32 h = 480);
...
//	OpenGL::Shader* _shadowPlaneProgram;
	SHADER_VARS(_background)
	SHADER_VARS(_actor)
	SHADER_VARS(_actorLights)
	SHADER_VARS(_sprite)
	SHADER_VARS(_dim)
	SHADER_VARS(_dimPlane)
	N3DS_3D::ShaderObj *_dimRegionShader;																				// DEFINITE?
	SHADER_VARS(_smush)
	void *_blankVBO, *_smushVBO, *_quadEBO;																// DEFINITE? - PARTIAL ADD
	SHADER_VARS(_text)
	SHADER_VARS(_primRect)
	SHADER_VARS(_primLines)
	N3DS_3D::ShaderObj *_irisShader;																					// DEFINITE?
	SHADER_VARS(_shadowPlane)
	SHADER_VARS(_manualClear)

	int _smushWidth;
	int _smushHeight;
	C3D_Tex _smushTex;
	void setupUntexturedQuad();																							// ADDED
