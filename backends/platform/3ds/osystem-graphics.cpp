...
#include "backends/platform/3ds/shaders/manualClear_shbin.h"
	...
	#include "backends/platform/3ds/shaders/playground3d/playground3d_viewport_shbin.h"
	#include "backends/platform/3ds/shaders/playground3d/playground3d_bitmap_shbin.h"
#endif
...
#define GAME_FORMAT _gfxState.gameFormat
#define GAME_WIDTH  _gfxState.gameWidth
...
static const TexMode _texmodeRGB5A1 = { Graphics::PixelFormat(2, 5, 5, 5, 1, 11, 6, 1, 0),
...
static const TexMode *texmodes[] = { &_texmodeRGBX8, &_texmodeRGB565, &_texmodeRGB5A1, &_texmodeRGBA4, &_texmodeCLUT8, &_texmodeRGBA8 };
...
void OSystem_3DS::init3DSGraphics() {
	...
	shaderDataMap.setVal("playground3d_viewport", ShaderData((u32*)const_cast<u8 *>(playground3d_viewport_shbin), playground3d_viewport_shbin_size));
	shaderDataMap.setVal("playground3d_bitmap", ShaderData((u32*)const_cast<u8 *>(playground3d_bitmap_shbin), playground3d_bitmap_shbin_size));
#endif
}
...
bool OSystem_3DS::setTexMode(TexModeID modeID) {
	switch (modeID) {
	case RGBA8:
	case RGB565:
	case RGB5A1:
...
OSystem::TransactionError OSystem_3DS::endGFXTransaction() {
	...
	if (_transactionDetails.formatChanged || _transactionDetails.sizeChanged || _transactionDetails.renderModeChanged) {
		...
		if (!setTexMode(TEXMODE_ID)) {
			...
			// Sprite
			_gameTopTexture.create(GAME_WIDTH, GAME_HEIGHT, TEXMODE, true);
			// Surface
			if (GAME_FORMAT == _gameTopTexture.format)
				_gameScreen.free();
			else
				_gameScreen.create(GAME_WIDTH, GAME_HEIGHT, GAME_FORMAT);
...
void OSystem_3DS::setPalette(const byte *colors, uint start, uint num) {
	...
	Graphics::convertPaletteToMap(_paletteMap + start, colors, num, _texmodeCLUT8.surfaceFormat);
...
void OSystem_3DS::copyRectToScreen(const void *buf, int pitch, int x,
	...
	if (GAME_FORMAT == Graphics::PixelFormat(2, 5, 5, 5, 0, 10, 5, 0, 0)) {
...
void OSystem_3DS::flushGameScreen() {
	if (GAME_FORMAT == _gameTopTexture.format) {
		return;
	} else if (GAME_FORMAT == Graphics::PixelFormat(2, 5, 5, 5, 0, 10, 5, 0, 0)) {
