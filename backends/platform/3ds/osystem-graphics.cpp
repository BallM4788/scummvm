...
static const TexMode _texmodeRGB555 = { Graphics::PixelFormat(2, 5, 5, 5, 1, 11, 6, 1, 0),
...
void OSystem_3DS::init3DSGraphics() {
	...
	_renderTargetTop = C3D_RenderTargetCreate(240, topScreenWidth, GPU_RB_RGBA8, -1);
	...
	_renderTargetBottom = C3D_RenderTargetCreate(240, 320, GPU_RB_RGBA8, -1);
	...
	//_gameTopTexture = new Sprite();
	//_gameBottomTexture = new Sprite();
	//_overlay = new Sprite();
	//_activityIcon = new Sprite();
	//_osdMessage = new Sprite();
	//_cursorTexture = new Sprite();
...
//			if (RENDER_MODE == OSystem::kGfxModeRender3d) {
//				C3D_FVec *fvecptr = C3D_FVUnifWritePtr(GPU_VERTEX_SHADER, 0, 1);
//				debug("bottom screen begin");
//				for (int i = 0; i < 20; i++) {
//					debug("%f,\t%f,\t%f,\t%f", fvecptr[i].x, fvecptr[i].y, fvecptr[i].z, fvecptr[i].w);
//				}
//				debug("bottom screen end");
//			}
...
void *OSystem_3DS::createBuffer(size_t size/*, size_t stride = NULL*/, const void *data) {
	void *ptr;

	// if (!stride) {
		ptr = linearAlloc(size);
		debug("new linear buffer - %lx", (u32)ptr);
		if (data == nullptr)
			memset(ptr, 0, size);
		else
