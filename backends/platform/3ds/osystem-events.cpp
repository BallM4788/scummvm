...
void OSystem_3DS::transformPoint(touchPosition &point) {
	if (!_overlayVisible) {
		point.px = static_cast<float>(point.px) / _screenBufferBottom.getScaleX() - _screenBufferBottom.getPosX();
		point.py = static_cast<float>(point.py) / _screenBufferBottom.getScaleY() - _screenBufferBottom.getPosY();
	}
...
void OSystem_3DS::clipPoint(touchPosition &point) {
	if (_overlayVisible) {
		...
		point.px = CLIP<uint16>(point.px, 0, _screenBufferTop.actualWidth  - 1);
		point.py = CLIP<uint16>(point.py, 0, _screenBufferTop.actualHeight - 1);
	}
}
